...
create or replace view public.team_sequence_agg as
select team, team_id,
  count(distinct game_id) as matches,
  round(count(*)::numeric/nullif(count(distinct game_id),0),1) as seqs_per_match,
  round(avg(n_pass)::numeric,2) as passes_seq,
  round(avg(dur_s)::numeric,1) as seconds_seq,
  round(avg(n_players)::numeric,2) as players_seq,
  round(avg(xt_sum)::numeric,4) as xt_seq,
  round(100.0*avg(low_build::int)::numeric,1) as low_build_pct,
  round(100.0*avg(high_build::int)::numeric,1) as high_build_pct,
  round(100.0*avg(structured::int)::numeric,1) as structured_pct,
  round(100.0*avg(very_short::int)::numeric,1) as very_short_pct,
  round(100.0*avg(long_ball::int)::numeric,1) as long_pct,
  round(100.0*avg(has_switch::int)::numeric,1) as switches_pct,
  round(100.0*avg(wide_triangles::int)::numeric,1) as wide_tri_pct,
  round(100.0*avg(hold_up::int)::numeric,1) as hold_up_pct,
  round(100.0*avg(ends_opp_half::int)::numeric,1) as ends_opp_half_pct,
  round(100.0*avg((end_third='def')::int)::numeric,1) as ends_def_third_pct,
  round(100.0*avg((end_third='att')::int)::numeric,1) as end_att_third_pct,
  round(100.0*avg(ended_in_box::int)::numeric,1) as end_in_box_pct,
  round(100.0*avg(end_around_box::int)::numeric,1) as end_around_box_pct,
  round(100.0*avg(finds_central::int)::numeric,1) as finds_central_pct,
  round(100.0*avg(finds_wide::int)::numeric,1) as finds_wide_pct,
  round(100.0*avg(ended_shot::int)::numeric,1) as ends_in_shot_pct,
  round(100.0*sum(n_prog_central)::numeric/nullif(sum(n_prog),0),1) as central_prog_share,
  round(100.0*sum(n_wide_pass)::numeric/nullif(sum(n_pass),0),1) as wide_pass_pct
from public.sequences where is_open_play group by team, team_id;

create or replace view public.team_sequence_style as
with u as (
  select a.team, m.metric, m.raw
  from public.team_sequence_agg a
  cross join lateral (values
    ('seqs_per_match',a.seqs_per_match),('passes_seq',a.passes_seq),('seconds_seq',a.seconds_seq),
    ('players_seq',a.players_seq),('xt_seq',a.xt_seq),('low_build_pct',a.low_build_pct),
    ('high_build_pct',a.high_build_pct),('structured_pct',a.structured_pct),('very_short_pct',a.very_short_pct),
    ('long_pct',a.long_pct),('switches_pct',a.switches_pct),('wide_tri_pct',a.wide_tri_pct),
    ('hold_up_pct',a.hold_up_pct),('ends_opp_half_pct',a.ends_opp_half_pct),('ends_def_third_pct',a.ends_def_third_pct),
    ('end_att_third_pct',a.end_att_third_pct),('end_in_box_pct',a.end_in_box_pct),('end_around_box_pct',a.end_around_box_pct),
    ('finds_central_pct',a.finds_central_pct),('finds_wide_pct',a.finds_wide_pct),('ends_in_shot_pct',a.ends_in_shot_pct),
    ('central_prog_share',a.central_prog_share),('wide_pass_pct',a.wide_pass_pct)
  ) as m(metric, raw)
)
select team, metric, raw,
  round(((raw - avg(raw) over (partition by metric)) /
         nullif(stddev_samp(raw) over (partition by metric),0))::numeric,2) as z
from u;
