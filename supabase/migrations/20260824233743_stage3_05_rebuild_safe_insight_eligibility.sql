...
begin
  ...
  -- a detector with no declared requirement must not run at all
  delete from public.insights i
  ...
  -- The delete above joins v_team_sample, so a club absent from it
  -- entirely matched nothing and survived. That is how cup-only clubs
  -- such as Mansfield and Bayern kept insights after cup scoping.
  delete from public.insights i
  where i.team is not null
    and not exists (
