...
begin
  ...
  -- 1) standout profile: tops his pool for a role
  ...
  -- 3) squad gap: no credible option in a role that matters for that position
  insert into public.insights (lens,category,scope,subject,subject_label,team,detector,headline,detail,metrics,evidence,score,confidence)
  ...
  best as (
    select pcr.team, t.pool, t.role, t.friendly, t.why,
      max(p.pct) as best_pct,
      (array_agg(p.player order by p.pct desc))[1] as best_player,
      count(*) as options
    from targets t
  ...
  -- 4) minutes-inflated: strong usage, but the minutes came in decided games
  insert into public.insights (lens,category,scope,subject,subject_label,team,detector,headline,detail,metrics,evidence,score,confidence)
  select 'sd','usage','player', r.player_id, r.player, r.team, 'minutes_inflated',
    ...
    jsonb_build_object('player_id',r.player_id,'team',r.team),
    (100 - r.leverage_pct), 'medium'
  from public.v_squad_role r where r.minutes_inflated;

  -- 5) misfit profile: plays one position, behaves like another
  insert into public.insights (lens,category,scope,subject,subject_label,team,detector,headline,detail,metrics,evidence,score,confidence)
  with freq as (
  ...
  where f.pct_of_pool <= 8 and a.primary_pct >= 75 and pcr.inv >= 200;

  -- ============================ TACTICAL LENS ============================

  -- 6) team profile: ALWAYS fires, one per club, so no side is invisible
  insert into public.insights (lens,category,scope,subject,subject_label,team,detector,headline,detail,metrics,evidence,score,confidence)
  with extremes as (
    select team, metric, z, abs(z) az,
      row_number() over (partition by team order by abs(z) desc) rk
  ...
  -- 10) progression route extremes
  insert into public.insights (lens,category,scope,subject,subject_label,team,detector,headline,detail,metrics,evidence,score,confidence)
  with r as (select *, rank() over (order by finds_central_pct desc) c_rk,
      rank() over (order by finds_wide_pct desc) w_rk, count(*) over () nteams from public.team_sequence_agg)
