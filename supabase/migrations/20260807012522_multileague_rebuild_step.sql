...
-- Per-league preflight. The contamination guard now checks a league's events against
-- that league's whitelist, so one league's teams can't be flagged against another's.
create or replace function public.preflight_league(p_league text default null)
returns text language plpgsql security definer set search_path = public as $fn$
declare r record; bad text; msgs text := '';
begin
  for r in select league from public.leagues where is_active
             and (p_league is null or league = p_league) loop
    if (select count(*) from public.team_names where league = r.league) = 0 then
      msgs := msgs || format('%s: no whitelist yet (skipped); ', r.league);
      continue;
    end if;
    ...
    msgs := msgs || format('%s ok; ', r.league);
  end loop;
  return coalesce(nullif(msgs,''), 'no active leagues');
end $fn$;
revoke execute on function public.preflight_league(text) from public, anon, authenticated;
grant execute on function public.preflight_league(text) to service_role;

-- rebuild_step gains an optional league argument. Analytics are still rebuilt whole
-- (the matviews span leagues by design), but preflight and reporting are per-league,
-- and the signature is in place for per-league partial rebuilds as volume grows.
...
as $fn$
declare seq_games int; evt_games int; null_xt int;
...
begin
  case p_step
    ...
    when 'verify' then
      ...
      select count(*) into n_srch from public.player_search;
      return format('verified -- %s sequences over %s games, %s outfield players, %s in search index, %s league(s)',
                    n_seq, seq_games, n_pcr, n_srch, (select count(*) from public.leagues where is_active));
