...
create table if not exists public.player_chain_roles (
  player_id text primary key,
  player text, team text, pos text,
  inv integer, player_xt numeric, hold_secs numeric,
  initiator numeric, bridge numeric, progressor numeric, carrier numeric, vertical numeric,
  support_angle numeric, individual numeric, creator numeric, box_threat numeric, finisher numeric
);
...
begin
  ...
  invv as (
    select player_id, player, team, xt_delta,
      (ord_a=1) f_init,
      (ord_a>1 and ord_a<seq_len and type='Pass' and outcome_type='Successful' and end_x>=x) f_bridge,
