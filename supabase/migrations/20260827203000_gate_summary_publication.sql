-- Publish browser-facing summary matviews only after error-level invariants pass.
begin;

create or replace function public.rebuild_step(p_step text, p_league text default null)
returns text language plpgsql security definer
set search_path to 'public' set statement_timeout to '240s'
as $function$
...
revoke all on function public.rebuild_step(text,text) from public,anon,authenticated;
grant execute on function public.rebuild_step(text,text) to service_role;

do $assert$ declare body text;
begin
  select pg_get_functiondef(p.oid) into body from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='public' and p.proname='rebuild_step' and pg_get_function_identity_arguments(p.oid)='p_step text, p_league text';
  if body not like '%summaries held pending verification%' then raise exception 'ASSERT FAILED. Insights still publishes summaries.'; end if;
  if body not like '%verify_rebuild()%refresh_site_summaries()%' then raise exception 'ASSERT FAILED. Verify does not precede publication.'; end if;
  if has_function_privilege('anon','public.rebuild_step(text,text)','EXECUTE') or has_function_privilege('authenticated','public.rebuild_step(text,text)','EXECUTE') then raise exception 'ASSERT FAILED. Public role can execute rebuild_step.'; end if;
end $assert$;
