...
alter table public.sequences add column if not exists xt_sum numeric;
...
begin
  ...
  with base as (
    select
      ...
      case when type='Pass' and outcome_type='Successful' and end_x is not null
           then coalesce(public.xt_val(end_x,end_y),0) - coalesce(public.xt_val(x,y),0)
           else 0 end as xt_delta,
  ...
  select
    ...
    case when max(x) filter (where ord_a=1) < 33.3 then 'def'
         when max(x) filter (where ord_a=1) < 66.7 then 'mid' else 'att' end as start_third,
    case when max(coalesce(end_x,x)) filter (where ord_d=1) < 33.3 then 'def'
         when max(coalesce(end_x,x)) filter (where ord_d=1) < 66.7 then 'mid' else 'att' end as end_third,
