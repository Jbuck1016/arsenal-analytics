...
as $fn$
declare seq_games int; evt_games int; null_xt int; bad_seq int;
...
begin
  case p_step
    ...
    when 'sequences' then
      ...
      -- build_sequences does not set the league column, so stamp it from the events
    ...
    when 'verify' then
      ...
      -- a sequence whose league disagrees with its own events means the stamp did not run
      select count(*) into bad_seq from public.sequences s
        join (select distinct game_id, league from public.events) e on e.game_id = s.game_id
        where s.league is distinct from e.league;
      if bad_seq > 0 then raise exception 'verify failed -- % sequences with the wrong league', bad_seq; end if;
...
-- La Liga whitelist, built from what actually landed. The guard now protects this league
-- instead of failing open on every match.
insert into public.team_names (event_name, match_name, display_name, league)
select distinct e.team, m.home_team, e.team, 'ESP-La Liga'
from public.events e
join public.matches m on m.game_id = e.game_id
where e.league = 'ESP-La Liga' and e.team is not null
  and not exists (select 1 from public.team_names t
                  where t.league='ESP-La Liga' and t.event_name = e.team)
group by e.team, m.home_team
