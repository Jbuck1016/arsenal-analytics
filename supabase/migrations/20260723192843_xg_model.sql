-- Empirical xG model fitted on this dataset (5,763 shots / 735 goals).
-- Cells are distance x angle x body-part x big-chance, with Bayesian shrinkage
-- toward the league conversion rate so thin cells stay sane.
-- Penalties are excluded from fitting and assigned a fixed historical value.
...
select
  ...
  degrees(
    case
      when (power((100-e.x)*1.05,2) + power((e.y-50)*0.68,2) - power(3.66,2)) <= 0 then pi()
      else atan( (7.32 * ((100-e.x)*1.05)) /
                 (power((100-e.x)*1.05,2) + power((e.y-50)*0.68,2) - power(3.66,2)) )
    end
  )                                                                    as angle_deg,
...
select
  s.d_bin, s.a_bin, s.is_header, s.is_bigchance,
  count(*) as n,
  sum(case when s.is_goal then 1 else 0 end) as goals,
  -- shrinkage: k pseudo-observations pulled toward the league base rate
  round(((sum(case when s.is_goal then 1 else 0 end) + 20 * g.base)
         / (count(*) + 20))::numeric, 4) as xg
from s cross join g
...
select
  f.game_id, f.ws_id, f.player_id, f.player, f.team, f.is_goal, f.is_open_play, f.is_pen,
...
left join mv_xg_bins b
 ...
 and b.is_header = f.is_header
 and b.is_bigchance = f.is_bigchance;
