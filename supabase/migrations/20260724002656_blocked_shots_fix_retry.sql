...
select
  ...
  e.qualifiers @> '[{"type":{"displayName":"Blocked"}}]'::jsonb      as is_blocked,
  case
    when e.is_goal then 'goal'
    when e.qualifiers @> '[{"type":{"displayName":"Blocked"}}]'::jsonb then 'blocked'
    when e.type='SavedShot'  then 'saved'
    when e.type='ShotOnPost' then 'post'
    else 'off'
  end                                                                as outcome
