-- =====================================================================
-- 20260824_03_scoping_invariants.sql
-- Stage 3, migration 03. APPLIED 2026-08-24. Requires 01 and 02.
-- Non-destructive. Adds the audited entry-object registry and the
-- structural plus output invariants that measure cup contamination.
--
-- Severity is warn deliberately. The contamination is real and these
-- checks report it honestly, but raising them to error before the
-- destructive rebuild lands would abort every scheduled scrape.
-- They become error in the destructive migration.
-- =====================================================================
begin;
...
alter table league_mart_entry_objects owner to postgres;
...
insert into invariants (name, description, check_sql, severity, enabled) values
(
  'league_mart_reads_scoped_sources',
  'Audited league-mart entry objects must read the canonical scoped sources rather than raw events, matches, sequences or lineups. Structural rather than value based: catches a new object that pools competitions before any wrong number is published.',
  'select count(*) from (
...
(
  ...
  'No domestic_cup or continental fixture may contribute to team metric grains.',
...
(
  ...
  'Every club in a registered league competition must resolve in mv_team_league, and no club may resolve to a league it never played in. Guards the silent MLS fallback class of defect.',
  'select
     (select count(*) from (
...
declare n int;
begin
  select count(*) into n from league_mart_entry_objects;
  if n <> 20 then raise exception 'ASSERT FAILED. Expected 20 audited entry objects, found %.', n; end if;
  select count(*) into n from invariants
  ...
  if n <> 4 then raise exception 'ASSERT FAILED. Expected 4 scoping invariants, found %.', n; end if;
  if has_table_privilege('anon','public.league_mart_entry_objects','SELECT') then
    raise exception 'ASSERT FAILED. Internal registry is anon readable.';
  end if;
end
$assert$;
