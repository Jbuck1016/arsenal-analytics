...
- The migration SHA-256 snapshot passes for all 192 frozen live-history migrations plus fourteen applied repository-authored migrations. The executable checker is canonical; this sentence must be updated whenever another later migration is added.
...
- The non-blocking atomic publication worker is production-verified. All 68 materialized views have eligible unique indexes, all refreshes in the three rebuild functions are concurrent, and queued run `7218a44b-f06a-4b68-ac68-473b351af435` committed 19 verified steps in 15m31.8s. Repeated in-flight and post-commit smoke tests kept all seven public surfaces available. The minute worker is executable by `service_role` only.
